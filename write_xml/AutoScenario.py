...
def write_hit_comments(hit_list):
    file_name, xml_path, path_ = split_path(comments_path)
    path_ = "{}/{}".format(path_,file_name + '_hit_comments.txt')
    if "(v2." in sys.version:
...
def write_unhit_comments(unhit_list):
    file_name, xml_path, path_ = split_path(comments_path)
    path_ = "{}/{}".format(path_, file_name + '_unhit_comments.txt')
    if "(v2." in sys.version:
        with open(path_, "w",) as f:
...
def write_hit_caseid(hit_caseid_list,):
    file_name, xml_path, path_ = split_path(comments_path)
    path_ = "{}/{}".format(path_, file_name + '_hit_caseid_list.txt')
    if "(v2." in sys.version:
        with open(path_, "w", ) as f:
...
def CreateXml(comments_list):
    ...
    for comm in comments_list:
        ...
        if ((len(cpp_c_list) <= line_num) and (len(cpp_c_list) > 0)):
            all_write_data += 1
            produce_info_dict = ProduceXML(case_num,cpp_c_list,case_id,case_owner,case_owner_alias,output,case_comments)
